 <?php 
	require_once("header-links.php");	
	  $sql = 'SELECT  c.*,showOrgInterventions4MisAnalyst(c.caseId,c.IomFormType) as OrgInterventions from  childGridViewData c where Iom_analysis_state=1';
// if($UserType!=436)	
																		...
																		<thead>
																				<tr>
																				 <th  data-field="childId" data-filter-control="input" data-sortable="true">الرقم</th>
																					...
																					<th  data-field="OrgInterventions" data-filter-control="input" data-sortable="true"> تدخلات المنظمة</th>
...
function IomSubmitFormatter(value, row, index) {
	 return [
        '<a class="like btnEditContract"  title="Like" href="'+checkPage(row.IomFormType)+row.caseId+'">',
...
function checkPage(TypeNo)
{
	return TypeNo==0?"iom_analysis.php?Case_No=":"iom_analysisEcnomic.php?Case_No=";
}
