 <?php 
	require_once("header-links.php");	
	 $sql = 'SELECT  c.*,getMostOfBehaviors(caseId) as behaviors,showSetSuggestPhysActList(caseId) as SuggestedActivities from  childGridViewData c   where Diagonse_state!=0 and Diagnose_closed is null';
if($UserType!=436)		
																				...
																				<tr>
																				 <th  data-field="childId" data-filter-control="input" data-sortable="true">الرقم</th>
																					...
																					<th  data-field="DiagnoseDate" data-filter-control="input" data-sortable="true"> تاريخ الانشاء</th>
																						<th  data-field="behaviors" data-filter-control="input" data-sortable="true"> الاعراض</th>
																						<th  data-field="SuggestedActivities" data-filter-control="input" data-sortable="true">الأنشطة المقترحة</th>
																					
																					<th data-formatter="SubmitFormatter" >تعديل الحالة</th>
...
<?php require_once("footer-links.php");  ?>
 ...
 $(function(){
  ...
  var m=$("#tblMdlPendingDiagnoseChildren").bootstrapTable('destroy');
		...
		{$.post('AjaxFiles/AjaxCase4diagnose.php',{State:1,FrmDate:$('#SearchFrm').val(),ToDate:$('#SearchTo').val()},function(r){
			  var m=$("#tblMdlPendingDiagnoseChildren").bootstrapTable('destroy');
