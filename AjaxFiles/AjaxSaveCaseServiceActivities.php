  ...
  if(isset($_POST["checkedServices"]))
   { $fields=array(":childCaseId",":serviceId",":startingDate",":endingDate",":quantity",":serviceStateId",":createdUserId",":ServiceGiverId");
     foreach($_POST["checkedServices"] as $cs ) {
          $values=array( check_data($ChildCaseId),check_data($cs['ServiceId']),check_data($cs['StartingDate']),check_data($cs['EndingDate']),check_data($cs['Quantity']),check_data($cs['ServiceState']),$_SESSION["user_id"],check_data($cs['ResponsiblePerson']));
